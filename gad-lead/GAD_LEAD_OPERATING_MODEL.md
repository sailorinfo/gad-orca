...
## 20. Proportional governance enforcement (LEAN-02)

Every ordinary Batch records `changeClass`, `risk`, governance profile (`P0`–`P3`) and one execution Profile (`QUICK`, `STANDARD`, `STRICT`, or `CRITICAL`). The strongest applicable constraint wins: P0 permits QUICK, P1 requires STANDARD, P2 requires STRICT, and P3 requires CRITICAL; C2/C3, R3, Human Authority, evidence-integrity, or recovery risk requires at least STRICT; C4, R4, or critical-control failure requires CRITICAL. Ambiguity selects the next stronger Profile. G3 freezes the selected Profile and no runtime downgrade is allowed.

The exact G3 decision object freezes nonempty, unique risks, required tests, required evidence, stop conditions, and budgets. Each risk maps to a frozen test. Profile verification is inherited: QUICK requires a deterministic check per risk and trusted smoke; STANDARD adds affected-path integration; STRICT adds complete requirement-to-implementation-to-evidence coverage and independent Review; CRITICAL adds independent Gate audit and explicit failure/recovery evidence. Trusted smoke requires a clean declared fixture, repeatable command, exit 0, its expected assertion, no skipped required assertion, and retained input, output, and status.

Before implementation changes, classify a failure as product, fixture, harness, environment, or evidence/control. Uncertainty stops. Fixture/harness failure neither becomes product failure nor expands tests unless the product behavior is reproduced on the trusted path. Before `REVIEW_PASS`, every required requirement must name reachable implementation, concrete evidence, and PASS verdict; unrelated passing tests cannot fill a missing row.

The default frozen budget is one implementation Worker, one Reviewer Session, one implementation Worktree, zero mechanical Workers, four dispatch/session records, one initial Review, one bounded Rework, one new Branch, and three total Worktrees including main and gad-lead. A second Review failure or any overrun stops. Re-review is allowed only after actual `REVIEW_FAIL` or approved material rework. When STRICT/CRITICAL Review is required and Orca cannot evidence fresh-Session file-system isolation, exactly one fourth Review Worktree is the fallback; otherwise the three-Worktree path applies. Review independence is never traded away.

`gad-control.ps1` action `governance-check` is read-only and deterministically checks this frozen object. It does not authenticate G3, Review, or evidence quality; Lead reconciles those authorities separately. `SIMPLIFY` may remove only optional coordination/hardening and may not lower Human Gate, independent Review, or evidence. Optional hardening stays outside the current Batch, and derived status remains unchanged until G5-authorized integration.
