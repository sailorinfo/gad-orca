...
Run `powershell -File gad-lead/tools/gad-control.ps1 -Package <action.json>` only after Lead reconciles the actual explicit Human Gate, its approved target and scope, and current project state. Caller-supplied approval text, even when committed and byte-checked, does not prove that Gate occurred. The JSON package names the intended Gate and exact target and expected objects; these fields never authenticate Human Gate or independent Review evidence. Lead must reconcile both from the real decision and review records before invocation. The tool checks Git/Orca facts and returns JSON preconditions, postconditions, change status, and recoverable error. Actions are `promote`, `integrate`, `status`, `terminal-close`, `worktree-remove`, `branch-delete`, `remote-check`, and `close`. A remote SHA check does not authorize a push.
