...
try{
 ...
 $identity="$DispatchId|$SessionId|$sha|$($check.id)|$($terminal.id)"
 $s2=[ordered]@{scenarioId='S2';frozenSha=$sha;fixtureIdentity='lean-final-s2-live-v2';external=$external;local=[ordered]@{action='implementation';exactObject=$sha;dispatchId=$DispatchId;sessionId=$SessionId;liveness='connected+writable'};capturedAt=$now;waitSeconds=0;observations=@([ordered]@{identity=$identity},[ordered]@{identity=$identity})}
